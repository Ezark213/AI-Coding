...
| 2.1.0 | 2026-01-29 | M365_Detailed.pptx完全準拠: 全位置情報を実ファイルから抽出して修正 |
| 2.2.0 | 2026-01-29 | 高級感デザインガイドライン追加: 直角、2色制限、白背景＋細ボーダー、上部アクセントライン |
| **2.2.1** | **2026-01-29** | **テキスト強調ルール追加: Markdown風の\*\*強調\*\*禁止、赤太文字（#C00000）で数値・重要ワードを強調** |

---

## 高級感デザインガイドライン（v2.2.0追加）

**経営層向け提案書にふさわしい、信頼感・高級感のあるデザイン指針です。**

### ⚠️ 絶対に守るべき原則

| 原則 | 理由 |
|------|------|
| **角丸を使わない** | 直角（RECTANGLE）はビジネスライク、角丸は親しみやすいがカジュアルな印象 |
| **色は2色まで** | ティール＋グレーに統一。多色使いは安っぽく見える |
| **ベタ塗りより白背景＋細ボーダー** | 上品で洗練された印象になる |
| **枠線は細く（1-1.5pt）** | 太枠線（3pt以上）はカジュアルな印象 |
| **余計な装飾を避ける** | シンプルなほど高級感が出る |

### 高級感カラーパレット

| 要素 | カラー名 | HEX | RGB | 用途 |
|------|----------|-----|-----|------|
| **メイン** | ティール | **#0086AB** | (0, 134, 171) | タイトル、アクセント |
| **本文** | ダークグレー | **#333333** | (51, 51, 51) | 本文テキスト（濃いめ） |
| **補助文字** | グレー | **#666666** | (102, 102, 102) | サブタイトル、説明文 |
| **枠線** | ライトグレー | **#C8C8C8** | (200, 200, 200) | ボックス枠線 |
| **背景** | 白 | **#FFFFFF** | (255, 255, 255) | カード背景 |
| **強調** | 赤 | **#C00000** | (192, 0, 0) | 数値・重要ワードの強調（太字） |

> ⚠️ **禁止**: 派手なオレンジ（#ED7D31）、緑（#70AD47）、紫（#7030A0）をラベルやボックスに使わない
> ⚠️ **赤は強調専用**: 赤（#C00000）はテキスト内の数値や重要ワードの強調にのみ使用。ボックスやラベルには使わない

### テキスト強調ルール

**数値や重要ワードを強調する場合は、赤太文字（#C00000）を使用します。**
...
```python
# 強調色
EMPHASIS = RGBColor(192, 0, 0)  # 赤 #C00000

# テキスト内で一部を強調する場合
run = p.add_run()
run.text = "2〜3時間"
run.font.bold = True
run.font.color.rgb = EMPHASIS
```
...
```
┌────────────────────────────────────┐  ← 上部アクセントライン（ティール、高さ0.3cm）
│                                    │
│  1  タイトルテキスト                │  ← 番号は太字テキスト（サークル不要）
│                                    │
│     説明文テキスト                  │
│                                    │
└────────────────────────────────────┘  ← 直角、白背景、細ボーダー（1pt）
...
- 形状: **RECTANGLE**（角丸ではない）
- 背景: **白（#FFFFFF）**
- 枠線: **ライトグレー（#C8C8C8）、1pt**
- 上部アクセント: **ティール（#0086AB）、高さ0.3cm**
- 番号: **太字テキスト**（塗りつぶしサークル不要）
...
│  課題    │  説明テキスト...
└──────────┘
  ↑ 白背景、ティール枠線（1.5pt）、ティール文字
```

> ⚠️ **禁止**: 赤/緑/オレンジ/紫など5色を使い分けるデザイン → 全てティール枠に統一

### KPIボックスの配置ルール

**❌ 禁止パターン**: 箇条書きの横にポツンとKPIボックスを配置

```
❌ ダサい例：
• 箇条書き1              ┌─────────┐
• 箇条書き2              │ 2〜3時間 │  ← 孤立したボックスは違和感
• 箇条書き3              └─────────┘
```

**✅ 推奨パターン**: KPIは専用スライドで3-4個並べて表示、または箇条書き内に数値を含める
...
- 形状: **RECTANGLE**（直角）
- ステップラベル: テキストのみ（塗りつぶしサークル不要）
- 矢印: **ティール（#0086AB）**（オレンジではない）
...
### 追加カラー定義（図解用）※高級感デザインでは使用を控える

| 要素 | カラー名 | HEX | RGB | 用途 |
|------|----------|-----|-----|------|
| オレンジ | アクセント | #ED7D31 | (237, 125, 49) | ガントバーのみ |
| 赤枠 | クライアント側 | #C00000 | (192, 0, 0) | 体制図（顧客側）のみ |
| 青枠 | 当社側 | #0070C0 | (0, 112, 192) | 体制図（自社側）のみ |
...
### 5. KPIボックス（KPI Highlight Box）- 高級感版

重要な数値を強調表示するボックスです。**高級感デザインでは統一されたスタイルを使用します。**
...
- 形状: **RECTANGLE（直角）**
- 背景: **白（#FFFFFF）**
- 枠線: **ティール（#0086AB）、1.5pt**
- 上部アクセントライン: **ティール、高さ0.3cm**
- 数値: 44pt、太字、**ティール**
- ラベル: 18pt、ダークグレー

> ⚠️ **禁止**: 色分け（ティール/オレンジ/赤/緑）→ 全てティールに統一
...
**Before/After仕様（高級感版）**
- Beforeヘッダー: **ダークグレー（#666666）**
- Afterヘッダー: **ティール（#0086AB）**
- Before背景: **白（#FFFFFF）**、枠線グレー
- After背景: **白（#FFFFFF）**、枠線ティール
- 矢印: **ティール（#0086AB）**

> ⚠️ 赤/緑の対比は派手になりがち。グレー/ティールの対比で落ち着いた印象に
...
**プロセスフロー仕様（高級感版）**
- ステップボックス: 13cm × 12cm、**直角（RECTANGLE）**
- 番号: **テキストのみ**（サークル不要）
- 上部アクセントライン: ティール、高さ0.3cm
- 矢印: **ティール（#0086AB）**（オレンジではない）
- 背景: 白、枠線ライトグレー

> ⚠️ 塗りつぶしサークルや角丸はカジュアルな印象になるため避ける
...
## Modern Card Design（モダンカードデザイン）- 高級感版

v2.2.0で更新された、高級感・信頼感のあるカードデザイン仕様です。

### カラー定義（高級感版）

| 要素 | カラー名 | HEX | RGB |
|------|----------|-----|-----|
| カード背景 | **白** | **#FFFFFF** | (255, 255, 255) |
| カード枠線 | ライトグレー | **#C8C8C8** | (200, 200, 200) |
| 上部アクセント | ティール | **#0086AB** | (0, 134, 171) |

### 高級感カード要素

```
┌────────────────────────────────────┐  ← 上部アクセントライン（ティール、0.3cm）
│ ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓ │
├────────────────────────────────────┤
│                                    │
│  1  カードタイトル                  │ ← 番号はテキスト（サークル不要）
│                                    │
│     説明テキスト                    │ ← ダークグレー
│                                    │
└────────────────────────────────────┘
  ↑ 白背景、枠線: #C8C8C8（1pt）、直角
```
...
> ⚠️ **禁止**: 角丸（ROUNDED_RECTANGLE）、番号サークル、太枠線（3pt以上）
