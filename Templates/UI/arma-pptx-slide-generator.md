...
| **2.2.0** | **2026-01-29** | **高級感デザインガイドライン追加: 直角（角丸禁止）、2色制限（ティール＋グレー）、白背景＋細ボーダー（1-1.5pt）、上部アクセントライン、KPIボックス配置ルール（孤立配置禁止）、番号サークル廃止** |
...
> ⚠️ **禁止色**: 派手なオレンジ（#ED7D31）、赤（#C00000）、緑（#70AD47）、紫（#7030A0）をラベルやボックスに使わない
